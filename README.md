...
2. **menus** (메뉴판)
  - id (PK) : 메뉴판 고유번호
  - store_id (FK → Stores.id) : 어떤 매장에 속하는지 연결
  - name : 메뉴판 이름
  - description : 메뉴판 설명
  - created_at : 생성일
  - updated_at : 수정일
...
3. **menu_items** (메뉴 아이템)
  - id (PK) : 메뉴 아이템 고유번호
  - menu_id (FK → Menus.id) : 어떤 메뉴판에 속하는지 연결
  - name : 메뉴 이름 (예: 아메리카노, 카푸치노)
  - description : 메뉴 설명
  - price : 가격
...
4. **item_ingredients** (메뉴 재료)
  - id (PK) : 재료 고유번호
  - item_id (FK → Menu_Items.id) : 어떤 메뉴 아이템에 속하는지 연결
  - ingredient_name : 재료 이름 (예: 원두, 우유, 시럽)
